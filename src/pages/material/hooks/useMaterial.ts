...
export const useMaterials = (
	condition: MaterialConditionSearch,
	accessToken: string
) => {
	const [filter, setFilter] = useState(condition);

	const { data, isLoading, refetch, isError, error } = useQuery({
		queryKey: [queryKeys.materials,filter],
		queryFn: async () => {
			//สร้าง string query
	...
	return {
		data: data as MaterialResponses,
		isLoading,
		setFilter,
		refetch,
		isError,
 ...
 * @returns
 */
export const useMaterial = (id: number, accessToken: string) => {
 ...
 * @returns
 */
export function useMaterialImage(filename: string) {
